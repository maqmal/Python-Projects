...
class Transformer_XL(object):
    ########################################
    # initialize
    ########################################
    def __init__(self, checkpoint, is_training):
        # load dictionary
    ...
    ########################################
    def extract_events(self, input_path, emotion):
    ...
    # generate
    ########################################
    def generate(self, n_target_bar, temperature, topk, output_path, emotion, prompt=None):
        # if prompt, load it. Or, random start
        if prompt:
            events = self.extract_events(prompt, emotion)
            words = [[self.event2word['{}_{}'.format(e.name, e.value)] for e in events]]
            em_start = 'Emotion-Start_{}'
            words[0].append(self.event2word[em_start.format(emotion)])
        ...
        while current_generated_bar < n_target_bar:
            ...
            if word == self.event2word['Bar_None']:
                print("Generated word for bar: {}".format(current_generated_bar))
                current_generated_bar += 1
            # re-new mem
            batch_m = _new_mem
        em_end = 'Emotion-End_{}'
        words[-1].append(self.event2word[em_end.format(emotion)])
        # write
    ...
    def prepare_data(self, midi_paths):
        ...
        for path in midi_paths:
            ...
            events = self.extract_events(path, emotion)
